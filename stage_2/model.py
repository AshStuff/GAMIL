...
class GNN_Module(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, bias, bn, add_loop, lin=True, gcn_name='SAGE', sync=False
        ...
        self.gcn1 = self._gcn(gcn_name, input_dim, hidden_dim, bias,
                              activation)  # DenseSAGEConv(input_dim, hidden_dim, normalize= True, bias= bias)
        self.active1 = self._activation(activation)
        self.gcn2 = self._gcn(gcn_name, hidden_dim, hidden_dim, bias,
                              activation)  # DenseSAGEConv(hidden_dim, hidden_dim, normalize= True, bias= bias)
        self.active2 = self._activation(activation)
        self.gcn3 = self._gcn(gcn_name, hidden_dim, embedding_dim, bias,
                              activation)  # DenseSAGEConv(hidden_dim, embedding_dim, normalize=True, bias= bias)
...
class SoftPoolingGcnEncoder(nn.Module):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, bias, bn, assign_hidden_dim,
        ...
        self.pred_model = self.build_readout_module(pred_input, pred_hidden_dims, activation)
    ...
    def build_readout_module(self, pred_input_dim, pred_hidden_dims, activation):
        pred_input_dim = pred_input_dim
        if len(pred_hidden_dims) == 0:
            pred_model = nn.Linear(pred_input_dim, pred_dim)
        else:
            pred_layers = []
...
class GAMIL(nn.Module):
    def __init__(self, num_graphs, max_num_nodes, input_dim, hidden_dim, num_classes, embedding_dim, bias, bn, assign_hidden_dim,
                 assign_ratio=0.25, pred_hidden_dims=[50], concat=True, gcn_name='SAGE',
                 collect_assign=False, load_data_sparse=False, norm_adj=False,
                 activation='relu', drop_out=0., jk=False):
        super().__init__()
        for i in range(num_graphs):
            self.add_module(f"graph_{i}",
                            SoftPoolingGcnEncoder(max_num_nodes, input_dim, hidden_dim, embedding_dim, bias, bn,
                                                  assign_hidden_dim,
                                                  assign_ratio=assign_ratio, pred_hidden_dims=pred_hidden_dims,
                                                  concat=concat,
                                                  gcn_name=gcn_name,
                                                  collect_assign=collect_assign,
                                                  load_data_sparse=load_data_sparse,
                                                  norm_adj=norm_adj,
                                                  activation=activation,
                                                  drop_out=drop_out, jk=jk))
    ...
    def forward(self, data):
        ...
        for i, each_data in enumerate(data):
            out = self._modules[f"graph_{i}"](each_data)
            H.append(out.unsqueeze(1))

        H = torch.cat(H, dim=1)
        A = self.attention(H)  # NxK
        # A = torch.transpose(A, 1, 0)  # KxN
        A = F.softmax(A, dim=1)  # so   ftmax over N

        M = torch.bmm(A.transpose(2,1), H).squeeze(1)  # KxL
        y = self.classifier(M)
        return y
