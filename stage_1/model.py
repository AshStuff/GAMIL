...
class TextureEncoder(nn.Module):
    def __init__(self, D, K):
        super().__init__()
        self.encoder = Encoding(D, K)
        self.fc = nn.Linear(D*K, 128)
    def forward(self, x):
        encoder = self.encoder(x)
        encoder = encoder.view(encoder.shape[0], -1)
        fc = self.fc(encoder)
        out = normalize(fc, p=2.0)
        return out
...
class AutoEncoderVGG(nn.Module):
    ...
    def __init__(self, pretrained_params=True):
        ...
        self.texture_encoder = TextureEncoder(D=512, K=32)
    ...
    def forward(self, x):
        ...
        encoder_out = self.texture_encoder(code)
        x_prime = self.decoder(code, pool_indices)

        return x_prime, encoder_out
