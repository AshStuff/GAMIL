...
class BatchCriterion(nn.Module):
    ...
    def __init__(self, negM, T, batchSize):
        ...
        self.negM = negM
        self.T = T
        self.diag_mat = 1 - torch.eye(batchSize * 2) # ToDo: change to cuda
...
class CombinedLoss(nn.Module):
    def __init__(self, negM, T, batchSize):
        ...
        self.batch_criterion = BatchCriterion(negM, T, batchSize)
    def forward(self, inputs, features, targets):
        unsup_loss = self.batch_criterion(features, None)
        mse_loss = self.mseloss(inputs, targets)
        loss = unsup_loss + mse_loss
        return loss
